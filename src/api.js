...
export const useFilterBasedOnDelivery = (isFreeDeliveryEnabled) => {
    const { isLoading: delivery, error: deliveryError, data: freeDeliveryProducts } = useQuery({
        queryKey: ['freeDeliveryProducts'],
        queryFn: () =>
            axios.get(
                `https://search.basalam.com/ai-engine/api/v2.0/product/search?from=0&f.freeShipping=${isFreeDeliveryEnabled}&filters.slug=handmade-leather-accessory&dynamicFacets=true&size=48&adsImpressionDisable=false&userCityId=1722`
            ),
        select: (res) => res.data // Add this line to ensure 'data' property is available
    });

    return { isLoading: delivery, error: deliveryError, data: freeDeliveryProducts };
};

export const useFilterBasedOnBestSeller =(isFreeDeliveryEnabled,isBestSellerEnabled)=>{
    ...
    return {isLoading:bestSeller, error:bestSellerError, data: bestSellerProducts}
}
// export const useGetAllProduct = (setOriginalProductList,setDisplayedProduct) =>{
...
//         select: (res) => res.data,
//     });
//     return { isLoading, error, data: productss }
// }
